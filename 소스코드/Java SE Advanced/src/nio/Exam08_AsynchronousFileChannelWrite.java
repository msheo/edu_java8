...
public class Exam08_AsynchronousFileChannelWrite {
	...
	public static void main(String[] args) {
		...
		// 이 for문의 핵심!! 비동기 이지만 프로세서는 blocking 하고 있어!!!!!
		for (int i = 0; i < 10; i++) {
			// 2. 파일 경로를 생성
			...
			try {
				...
				// 동기통신에서는 위에서 기다렸다가 해서 close하면 되는데.. 쓰레드로인한 병렬처리는?? 어떻게하지?? -> 스레드 다 끝나면 스레드 만든 곳으로 이벤트를 발생시켜(콜백)
				...
				// 비동기처리로 스레딩 병렬처리 끝나고 받아올때까지 기다리네?? 여기가 바로 blocking 하고있는 곳! 즉 비동기통신이지만 blocking !
				asyncFileChannel.write(buffer, 0, attachment, handler);
